...
5. I will be adding firstseen and lastseen values to the findings lookup to tell you when the first and last time the pair of `impacted asset` and `indicator` was seen.
6. Small dashboard tweaks are coming as well as some additional visual features to better visualize all the data AutoHunter collects.
7. I will be uploading a completed IOC lookup of around 2000 sources that you can use to immediately load IOC data without rebuilding the entire RSS cache from scratch. This will probably cause the "Total Number of Articles" to be less than the "Number of Articles Parsed" in the AutoHunts dashboard because you will be directly loading IOCs instead waiting for AutoHunter to parse the sources itself. Gets you hunting faster and doesn't require all the web traffic to get started, more on this soon.
